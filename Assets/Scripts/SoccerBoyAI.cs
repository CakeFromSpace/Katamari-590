...
public class SoccerBoyAI : AI
{
    ...
    void ChaseBall()
    {
        Vector3 direction_of_soccer_ball = soccer_ball.transform.position - transform.position;
        if(direction_of_soccer_ball.magnitude < looking_distance)
        {
            Vector3 push = Vector3.Normalize(soccer_ball.transform.position - transform.position) * push_strength * 10;
            push.y = 0;
            soccer_ball.GetComponent<Rigidbody>().AddForce(push);
            return;
        }
        direction_of_soccer_ball = Vector3.Normalize(new Vector3(direction_of_soccer_ball.x, 0, direction_of_soccer_ball.z));

        // lerp rotation away from soccer_ball and move at double speed
        transform.rotation = Quaternion.Slerp(transform.rotation, Quaternion.LookRotation(direction_of_soccer_ball), 1);
        transform.position += (2 * velocity * direction_of_soccer_ball) * Time.deltaTime;
    }
