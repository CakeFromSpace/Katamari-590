...
public class TankAI : AI
{
    ...
    void Shoot()
    {
        // can shoot twice before changing direction, only fires when there is no projectile
        if(direction_timer > change_direction_time / 2 && current_projectile == null)
        {
            // instantiates it slightly within the tower, with the rotation of the tower
            current_projectile = Instantiate(projectilePrefab, tower.transform.position + new Vector3(0, 20, 0), Quaternion.LookRotation(tower.transform.forward));
        }
    }
