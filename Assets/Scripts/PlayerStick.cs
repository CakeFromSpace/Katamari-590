...
public class PlayerStick : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if(collision.collider.gameObject.tag == "pickup")
        {
            ...
            Vector3 loc = -1 * (transform.rotation*Vector3.Normalize(transform.position - other.transform.position)*this.gameObject.GetComponent<SphereCollider>().radius);

            Debug.Log(loc);
            
            SphereCollider s = this.gameObject.GetComponent<SphereCollider>();
            Collider c = other.gameObject.GetComponent<Collider>();
            if(Vector3.Magnitude(s.bounds.size) > Vector3.Magnitude(c.bounds.size))
            {
                other.transform.position = transform.TransformPoint(loc);
                other.transform.parent = transform;
                other.tag = "sticky";
                s.radius += Vector3.Magnitude(c.bounds.size) / 1000;
